...
pub enum ProjectCreationError {
    TemplateTraversalError(template::TraversalError<template::RenderError>),
...
fn submodule_exists(root: &Path, name: &str) -> io::Result<bool> {
    ...
    if !path.exists() {
        ...
        util::read_str(&path)
            .map(|modules| util::has_match(&*SUBMODULE_NAME_RE, &modules, name))
    }
}
...
fn submodule_init(
    ...
    let submodule_exists = submodule_exists(root, name)
        .map_err(ProjectCreationError::GitSubmoduleStatusError)?;
    if !submodule_exists {
        let path = Path::new(&CONFIG.global.source_root).join(path.as_ref());
        let path_str = path.to_str().expect("`source_root` contained invalid unicode");
        util::git(&root, &["submodule", "add",
                "--name", name,
                remote, path_str,
            ])
...
pub fn hello_world(force: bool) -> Result<(), ProjectCreationError> {
    let template_dir = Path::new(concat!(env!("CARGO_MANIFEST_DIR"), "/templates"));
    let dest = CONFIG.project_root();
    let insert_data = |map: &mut template::JsonMap| {
        CONFIG.insert_data(map);
        let source_root = CONFIG.source_root();
        map.insert("source_root", &source_root);
    };
    let mut actions = template::traverse(
        &template_dir.join("project_root"),
        &dest,
        |path| template::render_path(path, insert_data),
    )?;
    actions.append(
        &mut template::traverse(
    ...
    template::process_actions(actions, insert_data)?;
    git_init(&dest)?;
    submodule_init(
        &dest,
        "rust_lib",
